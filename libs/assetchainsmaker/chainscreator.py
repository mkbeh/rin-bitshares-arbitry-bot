...
class ChainsCreator(BaseAssetChainsMaker):
    main_assets = ['BTS', 'BRIDGE.BTC', 'CNY', 'USD']
    _lock = RLock()
    ...
    def _create_chains_for_asset(asset, pairs):
                ...
                if main_pair[1] == secondary_pair[0]:
                    for end_pair in pairs_with_tertiary_assets:
                        if secondary_pair[1] == end_pair[0]:
                            chains.append([main_pair, secondary_pair, end_pair])

        return chains

    def start_creating_chains(self):
        pairs = self._get_pairs_from_file()
        self._run_in_multiprocessing(self._create_chains_for_asset, self.main_assets, pairs)
